...
- After locating the rollout, treat `rollout-summary` and `chunked-rollout-summary` as locator and triage output only. They may retain only a representative or last user record from a chunk and cannot prove that every later substantive human follow-up was preserved. For task reconstruction, use `chunk_meta` byte/record coordinates to select every relevant user-bearing chunk, fetch each exact range with `fetch-rollout-chunk`, reassemble split ranges when necessary, and hand the exact JSONL records to `codex-session-mining`. Do not batch multiple thread reads or stringify a raw result. Follow [Codex Thread Locator Skim](references/hosts.md#codex-thread-locator-skim) for the complete fallback and human-follow-up contract.
...
- `rollout-summary` and `chunked-rollout-summary` output text is signal-only, not raw prompt/tool output text. Use it only for coarse friction flags, coverage, and candidate selection; never use summary rows alone to decide which substantive user follow-ups existed.
- `fetch-rollout-chunk` writes one bounded byte range via `--byte-start`, `--byte-end`, and `--output <file>`; use `chunk_meta.fetch_ranges[]` when present, or the chunk `byte_start`/`byte_end` only when no split range is listed. For user-task reconstruction, fetch every selected user-bearing chunk even when `raw_fetch_recommended=false`, then pass the exact records to `codex-session-mining`.
...
- If `fetch-rollout` is blocked only by rollout size, use `chunked-rollout-summary` to locate candidate chunks, fetch every user-bearing chunk needed to reconstruct the request plus any `raw_fetch_recommended` ranges, and delegate the exact fetched records to `codex-session-mining`.
