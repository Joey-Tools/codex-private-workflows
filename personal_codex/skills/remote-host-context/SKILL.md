...
- Only an explicitly requested shadow Daily partial/backfill qualification may use `--emit holdout-receipt`. It requires `--qualification-mode shadow`, `--controlled-missing-host`, the status-issued host/window/source-kind/source-lease binding, and either a new run-local mode-0700 identity or `--require-existing-shadow-identity`.
