...
- `session-meta` accepts only complete newline-terminated JSONL records within its bounded prefix. It fails closed when the cap cuts through a record, even when the bytes through the cap form valid JSON, and when descriptor-relative directory enumeration is unreadable. Treat that explicit rollout error as a coverage gap; never reinterpret the missing row as proof that the rollout has no session metadata.
