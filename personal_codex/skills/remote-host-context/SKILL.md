...
- `session-meta` fails closed when its bounded prefix ends before valid metadata is found while the opened descriptor still has unread bytes. Treat that explicit rollout error as a coverage gap; never reinterpret the missing row as proof that the rollout has no session metadata.
