...
- After locating the rollout, treat `rollout-summary` and `chunked-rollout-summary` as locator and triage output only. They may retain only a representative or last user record from a chunk and cannot prove that every later substantive human follow-up was preserved. For full task reconstruction, never use summary rows to decide which chunks to omit: iterate every `chunk_meta` row in byte order, fetch every listed `fetch_ranges[]` entry or the row's full `byte_start`/`byte_end` range with sequential bounded `fetch-rollout-chunk` calls, reassemble split ranges without gaps or overlaps, and hand the complete exact JSONL record stream to `codex-session-mining`. Do not batch multiple thread reads or stringify a raw result. Follow [Codex Thread Locator Skim](references/hosts.md#codex-thread-locator-skim) for the complete fallback and human-follow-up contract.
...
- `chunked-rollout-summary` is the default large-rollout locator; it provides whole-rollout chunk coverage and bounded coordinates, not lossless semantic coverage.
...
- If `fetch-rollout` is blocked only by rollout size, use `chunked-rollout-summary` to enumerate the whole rollout, then fetch every range from every `chunk_meta` row in byte order and delegate the complete exact record stream to `codex-session-mining`; do not let lossy summary content choose omissions.
