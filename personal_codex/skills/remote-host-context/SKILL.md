...
- When Joey supplies a `codex://threads/<id>` URL, do not call `read_thread` unless the service supports and the caller sets server-side accepted item-type filtering, an item-count limit, and a whole-response byte cap. Require those three controls alongside one exact thread per call, `turnLimit: 1`, `includeOutputs: false`, and `maxOutputCharsPerItem` at most 400; the service itself must emit only selected thread metadata plus at most 12 short user or agent message snippets. Caller-side projection or truncation after receipt does not qualify. If any whole-response control is unavailable, as in the observed API, skip `read_thread` entirely: use bounded `session-meta` date windows plus exact session-id filtering, then `rollout-summary` or `chunked-rollout-summary`. If the date is unknown, use a bounded metadata-only exact-thread or index lookup, or ask for or derive a date; never widen `read_thread`. Do not batch multiple thread reads or stringify a raw result. Follow [Codex Thread Locator Skim](references/hosts.md#codex-thread-locator-skim) for the complete fallback and human-follow-up contract.
