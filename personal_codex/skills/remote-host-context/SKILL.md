...
- After locating the rollout, treat `rollout-summary` and `chunked-rollout-summary` as locator and triage output only. They may retain only a representative or last user record from a chunk and cannot prove that every later substantive human follow-up was preserved. For full task reconstruction, never use summary rows to decide which chunks to omit. Before any chunk fetch, iterate every `chunk_meta` row in byte order and validate the complete plan: every row must report the same `source_bytes` and `full_fetch_limit_bytes`, the globally ordered advertised ranges must cover exactly byte 0 through `source_bytes` without gaps or overlaps, total planned bytes must equal `source_bytes`, and `source_bytes` must be at most 16 MiB (16,777,216 bytes) with `full_reconstruction_allowed=true`. If the source exceeds that limit, fetch nothing; report the exact rollout, source byte count, and limit, then request Joey's explicit authorization for that exact rollout and byte count or use a future server-side lossless filter. Never silently loop over an over-limit plan. After explicit authorization, track cumulative exact range bytes, preserve gap-free non-overlapping coverage, and stop at the authorized `source_bytes`; abort if the source or plan changes. Otherwise, fetch every listed `fetch_ranges[]` entry or the row's full `byte_start`/`byte_end` range with sequential bounded `fetch-rollout-chunk` calls, then hand the complete exact JSONL record stream to `codex-session-mining`. Do not batch multiple thread reads or stringify a raw result. Follow [Codex Thread Locator Skim](references/hosts.md#codex-thread-locator-skim) for the complete fallback and human-follow-up contract.
...
- `chunked-rollout-summary` is the default large-rollout locator; it provides whole-rollout chunk coverage and bounded coordinates, not lossless semantic coverage. Each `chunk_meta` row exposes the common `source_bytes`, `full_fetch_limit_bytes`, and fail-closed `full_reconstruction_allowed` decision needed before any exact fetch.
...
- `fetch-rollout-chunk` writes one bounded byte range via `--byte-start`, `--byte-end`, and `--output <file>`. For full task reconstruction, first apply the complete-plan and 16 MiB authorization gate above. When allowed, consume all `chunk_meta` rows regardless of `raw_fetch_recommended`: use every `fetch_ranges[]` entry when present, otherwise fetch the row's full `byte_start`/`byte_end`, preserve global byte order, verify continuous coverage from byte 0 through `source_bytes`, and pass the complete exact record stream to `codex-session-mining`.
