...
- A later backfill must use real `session-shards` evidence for only the held-out host, preserve the exact window/source kind, name the partial run lineage, and replace the authenticated `holdout_ref`; another holdout cannot satisfy backfill. Receipt consumption and replacement must use the persistent atomic ledger transaction, not an in-memory consumed-ref set or separate writes.
- The Session Retrospective v2 shadow automation must route every coordinator invocation and accepted backfill through `scripts/session_retrospective_v2_shadow_runner.py`. The runner owns the command/finalize-phase allowlist, macOS pre-execution write sandbox, production/provider-state rejection, and per-host process/thread lock. It executes each status-authenticated `session-shards` capture, atomically publishes its owner-only transient stream, and calls the matching `accept-source` action under that same host lock; a runner, capture, or sandbox failure blocks the scenario.
