...
description: "Run Joey's local delivery gate for non-trivial repo changes: implement, build, test, update docs, form the landing commit, review its frozen exact head, then accept it. Use when wrapping up local work, probing local gate readiness, or starting a full workflow before PR readiness."
---
...
- Use one runtime/toolchain version unless Joey, repo policy, or the compatibility work itself requires a finite multi-version set.
