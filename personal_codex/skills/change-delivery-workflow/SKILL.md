...
- Named triple review 必须再包含 exact host `github.com` PR 当前 head 上的 exact `@codex review` 请求，以及 exact REST `chatgpt-codex-connector[bot]` / `Bot` 身份提供的完整 current-scope provider evidence。请求历史只控制 producer 是否可以再次发请求；consumer outcome 按 `$review-orchestration-playbook` 的 `github-codex-evidence-authority.md` 读取完整 comments、reviews、inline comments、threads 与 reactions 后判定，不要求 request/run attribution。Terminal comments/reviews 只能匹配 authority 的 fixed clean/finding/inline-parent grammar；其他 terminal-looking exact-provider payload 一律 malformed。Early/duplicate 请求只产生 `request_policy` warning，parent 仍不得主动发第二或第三条。`mixed` 始终以 trustworthy terminal payload 为权威；较晚的 `+1` 或 `eyes` 不会推翻 terminal selection，`eyes` 只会阻止 reaction-only fallback。`thumbs-up-clean` 需要来自 distinct non-current scopes 的 3–10 个 historical samples，exact current scope 必须单独校验且不能计入历史最小值。每个 historical sample 与 separate current snapshot 都要把 selected exact `@codex review` parent request 与 individual child exact-bot `+1` 的 IDs、URLs、immutable scope、server times 和严格顺序绑定，同时枚举所有 accepted same-scope requests 及其 fully paginated reactions；selected `+1` parent 必须是 semantic time 唯一最新的 request，防止 newer request 或 cross-parent `eyes`/conflict 被隐藏。Declaration authority 只来自 unchanged canonical GitHub REST issue-comment resource，并要求 exact bot/App/repository/PR/API-URL/HTML-URL binding 与 exact `If Codex has suggestions, it will comment; otherwise it will react with 👍.` line；generic issuer/source、caller record 和 self-hashed paraphrase 均无效。Exact App `chatgpt-codex-connector` 的 current-head post-request check/run 只能作为 service-start evidence；即使 `completed` / `success`，也不能完成 triple 或证明 clean/no-findings。无 PR、直接观察到任何 non-`github.com` host（包括 `sqbu-github.cisco.com` 与所有 GitHub Enterprise host），或 operating identity in `{hoteng, hoteng_cisco}` 时，明确报告 `effective double`。Fixed authority baseline 的 accepted structured capability/installation schema set 为空，no-start body grammar 也未启用，所以 integration/service uncertainty、缺失响应或 free-form provider prose 都不能降级。已启动且没有 accepted complete `thumbs-up-clean` reaction fallback 时，otherwise valid nonterminal/check-only evidence、missing terminal payload 或 retryable incomplete pagination remain `pending` while bounded waiting is meaningful；等待耗尽后才报告 `triple-inconclusive`。Malformed/stale evidence、ambiguous identity/scope、non-retryable failure 或 permanently incomplete enumeration 立即报告 `triple-inconclusive`；两类情况都不得降成 double。
