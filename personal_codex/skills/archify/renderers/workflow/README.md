...
```bash
node <loaded-skill-dir>/renderers/workflow/render-workflow.mjs input.workflow.json output.html
```

The renderer validates input against `<loaded-skill-dir>/schemas/workflow.schema.json`
...
```bash
node <loaded-skill-dir>/scripts/check-render-output.mjs output.html
```
...
`<loaded-skill-dir>/examples/agent-tool-call.workflow.json`; its `schema_version` selects
the applicable contract.

The schema lives at:

```text
<loaded-skill-dir>/schemas/workflow.schema.json
```
...
```bash
node <loaded-skill-dir>/bin/archify.mjs migrate workflow old.json new.json --to-schema 2 --json
```
...
```bash
node <loaded-skill-dir>/bin/archify.mjs validate workflow input.workflow.json --layout-json
```
