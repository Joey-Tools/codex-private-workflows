...
```bash
node <loaded-skill-dir>/renderers/dataflow/render-dataflow.mjs input.dataflow.json output.html
```

The renderer validates input against `<loaded-skill-dir>/schemas/dataflow.schema.json`
...
`<loaded-skill-dir>/examples/product-analytics.dataflow.json`.

The schema lives at:

```text
<loaded-skill-dir>/schemas/dataflow.schema.json
```
