...
- Capture bounded raw stdout in parent-owned state outside the model-visible worktree and pass it through [`validate_claude_stream.py`](../scripts/validate_claude_stream.py) with the parent-private accepted `--preflight-result`, exact resolved cwd, requested model, selected authentication source, and mandatory `--process-returncode <child-returncode>`. The validator must bind the selected version and current compatibility/profile digests before parsing the stream. Only `classification: accepted` with a zero child return code supplies a trusted result artifact; a missing, invalid, or nonzero child return code is inconclusive when stdout contains an otherwise valid-looking success result. A nonzero code preserves deterministic structured `blocked` or `blocked-authentication` evidence, while a bare exit code creates neither. Prose inspection, an ad hoc parser, partial output, or any fail-closed validator result cannot satisfy the lane. Validator acceptance does not attest the merged sandbox or determine the review outcome. Preserve its result string verbatim, then pass it to [`review_result.py`](../scripts/review_runtime/review_result.py) for canonical post-acceptance disposition. Prose inspection may select the helper's explicit content assessment only after acceptance; it never substitutes for the validator.
...
- Presentation is diagnostic metadata, not a retry signal. Never rerun solely because a result is extended, contradictory, ambiguous, or nonconforming. Rerun only after range/head evidence becomes invalid, a finding fix creates a new head, or Joey explicitly asks; otherwise surface an ambiguous disposition as the decision point.
