...
- A parent-owned control-order record proves both required local lanes had terminal artifacts before that request was written. A request posted earlier, or an unproved order, is `github-request-before-local-terminal` and leaves the GitHub lane `triple-inconclusive`; later local-lane completion does not cure it, its payload cannot count, and the unchanged head cannot receive a second request.
...
Prefer a GitHub review whose `commit_id` equals the current `headRefOid`. For same-head evidence available only as an issue comment, [the issue-comment-only correlation rule](github-pr-probes.md#issue-comment-only-correlation) is authoritative and supplements—not replaces—the exact-bot, complete-payload, lifecycle, range, and request-isolation requirements above. A terminal completion must bind through the exact request/run or the sole-unresolved/no-intervening fallback, and a no-start rejection must bind to the exact request/dispatch or satisfy that same fallback. The full exact current SHA may corroborate artifact scope, but it does not identify which request caused the response. An older request remains unresolved across a head change unless trustworthy correlated evidence resolves it; without the required correlation, that ambiguity is `triple-inconclusive` even when the terminal response names the current SHA.

Any push invalidates the old evidence and creates a new head epoch. Request at most one review for that new current head rather than reusing a stale comment; if an older request might still overlap and the artifact lacks explicit request/run binding, fail closed as `triple-inconclusive`. A current-SHA marker cannot disambiguate which request caused a result because an older request may execute after the push and review that same current head.

Effective-double fallback requires directly known no-PR/host/identity evidence or a properly request-bound authenticated result from exact `chatgpt-codex-connector[bot]` with REST `type: Bot` that proves the integration/service unavailable before any run starts. Exact current-head SHA binding alone is not request binding for completion or no-start evidence. Posting the request comment is not service start. Missing response, timeout, comment-write or generic HTTP failure, unknown provider/app identity, or evidence that proves neither unavailable nor started is `requested: triple`, `effective: triple-inconclusive`. Once acknowledgement, exact-App current-head post-request check/run activity, or review activity proves service start, malformed, stale, ambiguous, transiently incomplete, or check-only evidence is also triple-inconclusive and must not be converted to effective double, completed triple, or a clean result.
