...
3. Run `offline-frozen-diff-review` first through the stateful pinned Codex helper on the exact frozen range. Require its retained `preflight.json` before launching any separate Codex session; this proves the frozen workspace, diff, and prompt passed the sensitive-content and escaping-symlink checks. When a frozen base contains a registered synthetic test fixture, an explicit `--synthetic-secret-exemption <id>` is valid only if the helper's exact path/side/blob/rule/value/deleted-diff-occurrence binding passes and `preflight.json` records the applied ID; the rest of the diff remains fully scanned, and a path-only or test-directory allowlist is never valid evidence. If the reviewer runtime becomes unavailable only after that preflight, use the helper-retained frozen workspace for the configured clean-context `reviewer` fallback with the same evidence contract, collect the artifact, then run `stateful cleanup`. If the helper cannot complete the preflight itself, stop instead of bypassing it.
