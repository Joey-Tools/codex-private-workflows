...
A helper-defined synthetic-fixture exemption does not expand consent to real credentials. It must be selected explicitly by its registered ID, match the exact path, base-side blob OID, scanner rule, and synthetic value, fail closed on any drift, and appear in the retained preflight evidence. Never substitute a generic token allowlist, fixture-directory exclusion, or disabled base/diff scan.
