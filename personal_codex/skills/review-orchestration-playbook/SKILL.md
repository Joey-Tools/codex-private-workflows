...
- Avoid wide selected-file diffs as a default review tactic: do not start with `git diff --unified=30/40/50/60/80` or a whole-file `nl -ba` read when `--stat`, `--numstat`, changed-file lists, helper diff-file headers, or exact symbol windows would answer the question. When any selected-file diff, source read, or search returns a large result, such as 800+ lines or a tool-reported 10k+ original tokens, stop widening and re-scope with `--stat` / `--numstat`, one file or hunk at a time, and `rg -n` plus `sed -n '<start>,<end>p'`. For path-limited Git diff probes, put output/control options before `--`; use `git diff --name-only -- <paths>` or `git diff --stat -- <paths>`, never `git diff -- <paths> --name-only`.
