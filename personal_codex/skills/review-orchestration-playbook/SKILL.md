...
The explicit phrases `double review`, `双重 review`, `triple review`, and `三重 review` are contemporaneous user consent for scoped code-review egress to OpenAI and Anthropic. That consent covers any necessary tracked code in the named repository at the frozen head, the generated diff, and the review prompt/result sent to OpenAI Codex and Anthropic Claude Code. Triple review additionally authorizes current-head GitHub Codex review. In local-login mode, it also authorizes the helper to read only the account UUID, organization UUID, and email address from the owner-only host `~/.claude.json` and pass those three account-discovery fields to the Anthropic Claude process. It never covers the original file, credentials, settings, untracked files, unrelated repositories, other home-directory content, or broad workspace dumps. Read [egress-consent.md](references/egress-consent.md) before starting those lanes.
...
- Claude Code: when pinned native Mach-O Claude Code `2.1.202`, a native Mach-O `rg` executable, and macOS `sandbox-exec` are available for verified safe-mode probes and final process/network isolation, use the ordinary local Claude login by default with `claude-opus-4-8` and `max`; local-login mode additionally requires `/usr/bin/clang` to build its restricted native Keychain broker. The helper copies the digest-validated Claude artifact into a 0700 helper-owned runtime directory as a 0500 executable, revalidates the copied artifact and bundled-root set, and uses that same snapshot for identity/help probes, warmup, and every final attempt; later replacement of the installation path cannot change the launched artifact. Immediately before any warmup and independently immediately before every final model attempt, including the 4.8-to-4.7 entitlement fallback, the parent helper revalidates that snapshot, inspects the user, admin, and system trust domains, and composes a fresh helper-owned TLS bundle from its exact pinned bundled PEM roots, the system PEM, validated unconditional trust roots, and caller-provided certificate-only CA sources. It filters excluded SHA-1 roots and then verifies that every pinned SHA-256 root remains in the final bundle. Any excluded/pinned overlap, missing pinned root, executable/root-set mismatch, or snapshot drift fails closed. An earlier scan or bundle is never reused as current trust material. Original host CA files and directories are not exposed; `/System/Library/Keychains` is explicitly denied after the required `/System/Library` runtime allow, and other host Keychain locations receive no read allow. Only an otherwise valid access token whose expiry cannot cover the review window may trigger the fixed-input, no-tools safe-mode warmup; missing/malformed credentials, unsafe or changed account metadata, and unstable Keychain snapshots block before network launch. The final broker is read-only and OAuth refresh egress is blocked. An explicitly exported `ANTHROPIC_API_KEY` remains an optional authentication override and does not require the broker. Fall back to `claude-opus-4-7` with `max` only after an explicit account, plan, organization-policy, or model-entitlement denial for Opus 4.8. Script/wrapper installations and hosts without the required secure runtime are treated as unavailable.
- Trust flattening is fail-closed across the user, admin, and system domains. Claude Code is pinned to `CLAUDE_CODE_CERT_STORE=bundled`; the host system store is not treated as the trust authority. `/private/etc/ssl/cert.pem` is one filtered input to the helper-owned bundle. Built-in system-domain roots are inspected for deny/constraints but are not re-imported as custom roots. A non-empty non-deny trust-settings entry is never flattened, and constrained, missing, or nonconforming roots are omitted from helper-added material and caller snapshots unless that omission overlaps a pinned bundled root, which blocks the lane. An explicit deny remains a distinct hard stop, while malformed or unsupported trust data also blocks the lane. A missing or empty user/admin trust-settings array is accepted as an additional TrustRoot only after the referenced certificate is verified as a currently valid, strict self-signed CA root with critical `basicConstraints` and certificate-signing `keyUsage`. Retained trust evidence contains counts and disposition only, never certificate fingerprints or policy values.
- Local-login isolation bridges only Claude Code's three required account-discovery fields from an owner-only host `~/.claude.json`: account UUID, organization UUID, and email address. A bounded short-lived extractor validates and parses that file, emits only those three fields, and exits; excluded values never enter the long-lived helper process. The helper passes only those validated process fields to the Anthropic Claude process and never copies the original file or imports credentials, project settings, feature caches, or hooks. Executable probes receive none of the bridged fields. The helper does not intentionally write the values to the review prompt, structured evidence, or its own diagnostics, but complete Claude stdout/stderr is retained per attempt without field redaction; if Claude emits a value, that value remains in the retained output log.
- Local-login freshness is budgeted for the next model attempt only; every entitlement fallback revalidates the credential before starting. Expiry-shortfall is the only refreshable preflight outcome. Account metadata mismatch, credential instability, malformed content, and unsafe metadata-file state are integrity failures and never enter the networked warmup path. A warmup stores only bounded return-code/category/byte-count diagnostics plus a fixed-schema, allowlisted structural JSON summary; it never persists result/error text, model names, session identifiers, raw output, or credential content. A nonzero fixed-input error with omitted or empty model usage, no API status, and an auth-only bounded signal is treated as deterministic authentication. Omitted and malformed fields remain distinct; malformed usage or status never receives that promotion. Deterministic credential/authentication failures write `claude-blocked.json` with `status=blocked` and `reason_category=authentication`, while secure runtime/tool failures retain their separate unavailable/skip terminal state.
...
- `explicit-claude-review`, `double-review`, and `triple-review` authorize this helper's Anthropic Claude Code lane. If its native runtime, authentication, or pinned models are unavailable, that lane stops blocked or inconclusive; it never changes provider.
- Before any Codex or Claude-family egress, require the helper's escaping-symlink and sensitive-content preflight to pass. A blocked credential path or high-confidence secret pattern is a hard stop; remove the secret or narrow the review content instead of overriding the scan. The sole exception is an explicitly selected helper-defined synthetic-fixture exemption that is exact-path, base-blob, rule, value, and deleted-diff-occurrence bound, fails closed on drift, and is recorded in `preflight.json`; never add a generic token, directory, or test-tree allowlist.
- When the Claude-family helper needs approval, the escalation justification must repeat the explicit user request, exact repository, frozen `base_sha..head_sha`, Anthropic destination, included tracked-code/diff/prompt scope, and exclusions. Use the template in [egress-consent.md](references/egress-consent.md); a generic `run external reviewer` justification is insufficient.
