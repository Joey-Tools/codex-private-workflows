...
- Claude Code: when pinned native Mach-O Claude Code `2.1.202`, a native Mach-O `rg` executable, and macOS `sandbox-exec` are available for verified safe-mode probes and final process/network isolation, use the ordinary local Claude login by default with `claude-opus-4-8` and `max`; local-login mode additionally requires `/usr/bin/clang` to build its restricted native Keychain broker. Before warmup and final review, the parent helper inspects the user, admin, and system trust domains, then composes one helper-owned TLS bundle from the system PEM, verified unconditional trust roots, and caller-provided certificate-only CA sources after filtering. Original host CA files and directories are not exposed; `/System/Library/Keychains` is explicitly denied after the required `/System/Library` runtime allow, and other host Keychain locations receive no read allow. A stale token is refreshed only by a fixed-input, no-tools safe-mode warmup using ordinary Claude behavior; the final broker is read-only and OAuth refresh egress is blocked. An explicitly exported `ANTHROPIC_API_KEY` remains an optional authentication override and does not require the broker. Fall back to `claude-opus-4-7` with `max` only after an explicit account, plan, organization-policy, or model-entitlement denial for Opus 4.8. Script/wrapper installations and hosts without the required secure runtime are treated as unavailable.
...
- Local-login freshness is budgeted for the next model attempt only; every entitlement fallback revalidates the credential before starting. A warmup stores only bounded return-code/category/byte-count diagnostics and never persists its output or credential content.
