...
- Before any Codex or Claude-family egress, require the helper's escaping-symlink and sensitive-content preflight to pass. A blocked credential path or high-confidence secret pattern is a hard stop; remove the secret or narrow the review content instead of overriding the scan. The sole exception is an explicitly selected helper-defined synthetic-fixture exemption that is exact-path, base-blob, rule, and value bound, fails closed on drift, and is recorded in `preflight.json`; never add a generic token, directory, or test-tree allowlist.
