...
Prepare and validate each workspace through the active trusted helper contract in [review-workspace.md](references/review-workspace.md). Always clean it up after a terminal lane result unless Joey explicitly asks to retain it for diagnosis.
...
by hourly monitoring without a time limit. Report to Joey when the delay
...
When Joey authorizes implementation or PR repair, loop on the same explicit PR and current head:
