...
- Claude Code: when `ANTHROPIC_API_KEY` and macOS `sandbox-exec` are available for verified hook-free bare mode, use `claude-sonnet-5` with `max`; fall back to `claude-opus-4-8`, then `claude-opus-4-7`, both with `max`, under the same entitlement-only rule. OAuth/keychain authentication and hosts without the required read-only probe sandbox are intentionally treated as unavailable.
