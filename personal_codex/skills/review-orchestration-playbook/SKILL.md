...
- Claude Code: when pinned native Mach-O Claude Code `2.1.202`, a native Mach-O `rg` executable, and macOS `sandbox-exec` are available for verified safe-mode probes and final process/network isolation, use the ordinary local Claude login by default with `claude-opus-4-8` and `max`; local-login mode additionally requires `/usr/bin/clang` to build its restricted native Keychain broker. Immediately before any warmup and independently immediately before every final model attempt, including the 4.8-to-4.7 entitlement fallback, the parent helper inspects the user, admin, and system trust domains, then composes a fresh helper-owned TLS bundle from the system PEM, verified unconditional trust roots, and caller-provided certificate-only CA sources after filtering. An earlier scan or bundle is never reused as current authority. Original host CA files and directories are not exposed; `/System/Library/Keychains` is explicitly denied after the required `/System/Library` runtime allow, and other host Keychain locations receive no read allow. A stale token is refreshed only by a fixed-input, no-tools safe-mode warmup using ordinary Claude behavior; the final broker is read-only and OAuth refresh egress is blocked. An explicitly exported `ANTHROPIC_API_KEY` remains an optional authentication override and does not require the broker. Fall back to `claude-opus-4-7` with `max` only after an explicit account, plan, organization-policy, or model-entitlement denial for Opus 4.8. Script/wrapper installations and hosts without the required secure runtime are treated as unavailable.
- Trust flattening is fail-closed across the user, admin, and system domains. `/private/etc/ssl/cert.pem` is the fixed system baseline; built-in system-domain roots are inspected for deny/constraints but are not re-imported as custom roots. A non-empty non-deny trust-settings entry is never flattened. Because Claude's supported custom certificate stores are additive, any constrained, missing, or nonconforming root that would require exclusion blocks the lane before a bundle or caller snapshot is created. An explicit deny remains a distinct hard stop, while malformed or unsupported trust data also blocks the lane. A missing or empty user/admin trust-settings array is accepted as an additional TrustRoot only after the referenced certificate is verified as a currently valid, strict self-signed CA root with critical `basicConstraints` and certificate-signing `keyUsage`. Retained trust evidence contains counts and disposition only, never certificate fingerprints or policy values.
- Local-login freshness is budgeted for the next model attempt only; every entitlement fallback revalidates the credential before starting. A warmup stores only bounded return-code/category/byte-count diagnostics plus a fixed-schema, allowlisted structural JSON summary; it never persists result/error text, model names, session identifiers, raw output, or credential content. A nonzero fixed-input error with omitted or empty model usage, no API status, and an auth-only bounded signal is treated as deterministic authentication. Omitted and malformed fields remain distinct; malformed usage or status never receives that promotion. Deterministic credential/authentication failures write `claude-blocked.json` with `status=blocked` and `reason_category=authentication`, while secure runtime/tool failures retain their separate unavailable/skip terminal state.
