...
- A helper-backed subagent/internal lane is not equivalent to the independent Codex CLI PR review prompt that Joey runs from outside the parent workflow. Do not substitute one for the other in PR readiness gates.
...
- When a selected-file diff or source read still returns a large result, such as 800+ lines or a tool-reported 10k+ original tokens, stop widening with `git diff --unified=60/80` or whole-file `nl -ba`. Re-scope with `--stat` / `--numstat`, one file or hunk at a time, and read exact symbol windows with `rg -n` plus `sed -n '<start>,<end>p'`. For path-limited Git diff probes, put output/control options before `--`; use `git diff --name-only -- <paths>` or `git diff --stat -- <paths>`, never `git diff -- <paths> --name-only`.
