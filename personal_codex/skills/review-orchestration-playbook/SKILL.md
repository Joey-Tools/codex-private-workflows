...
- Local-login isolation bridges only Claude Code's three required account-discovery fields from an owner-only host `~/.claude.json`: account UUID, organization UUID, and email address. The helper passes only those validated process fields to the Anthropic Claude process and never copies the original file or imports credentials, project settings, feature caches, or hooks. Executable probes receive none of the bridged fields. The helper does not intentionally write the values to the review prompt, structured evidence, or its own diagnostics, but complete Claude stdout/stderr is retained per attempt without field redaction; if Claude emits a value, that value remains in the retained output log.
