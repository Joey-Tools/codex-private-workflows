...
- Default host scope follows `$remote-host-context`: local machine, `miku-bot-dev`, `hoteng-srv-01`, and `codex-hoteng-srv-01`.
...
Pass repeated `--source HOST=PATH` values when remote evidence has been materialized locally. `PATH` may be a Codex home containing `sessions/` or a task-scoped directory containing copied `rollout-*.jsonl` files. Retained host labels are restricted to `local`, the default remote hosts (`miku-bot-dev`, `hoteng-srv-01`, and `codex-hoteng-srv-01`), and `custom_source`; any other `HOST` label is bucketed as `custom_source` before retained artifacts are written.
