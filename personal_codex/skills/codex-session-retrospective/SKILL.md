...
- Read session metadata through raw chunks whose per-call and cumulative byte counts stay within the scan cap. Accept a complete final JSON record at EOF without a trailing LF, but fail closed before parsing when bytes remain beyond an exact cap boundary.
