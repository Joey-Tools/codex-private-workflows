...
- Resolve `SKILL_AUTHORING_DIR` from the loaded `$joey-skill-authoring` directory and `SKILL_CREATOR_DIR` from the loaded `$skill-creator` directory; do not assume either skill lives under the default home.
...
- Prefer `python3 "$SKILL_AUTHORING_DIR/scripts/codex_skill_validate.py" ...` to validate one or more skills through the local wrapper; do not edit or mirror OpenAI's validator for Joey-specific workflow changes.
