...
PR target authorization 是独立 preflight。`full workflow`、`merge-ready`、`在合并前停止`、`stop before merge`、`开 PR`、`开 PR 合进去`、`三重 review` / `triple review` 或类似措辞，只在目标 repository 明显由 Joey owns，或属于 Joey 明确配置的 default-authorized owners/repos 时，才授权 push 分支并创建/更新 PR。其他目标 repository 必须先停下来请求明确确认；确认问题必须列出 exact target repository/repositories、base branch、head repository/branch 和 draft/ready 状态，让 Joey 能判断是否允许。
...
- 在 push 分支或创建/更新 PR 前完成 PR target authorization preflight：如果 target repository 不明显由 Joey owns，也不在 Joey 明确配置的 default-authorized owners/repos 内，先停止并请求明确确认。确认问题必须列出 exact target repository/repositories、base branch、head repository/branch 和 draft/ready 状态；多 repo 时逐一列出。
...
- 读取 [review-lane-contracts.md](references/review-lane-contracts.md) 并保留其中 independent review prompt 和 evidence-budget contract，尤其是 `git diff --unified=30/40/50/60/80` / `git diff --function-context` / `git diff -W`、低上下文 multi-file selected diff、`git show <rev>:<path>`、`cat <file>`、整文件 `nl -ba`、`path-wide / multi-file / large-alternation raw rg -n`、single-file broad-pattern `rg -n`、`rg -n -C context search`、`800+ 行或 10k+ original tokens`、`git status --short --untracked-files=no`、`rg -l` / `rg --count`，以及 first-stage changed-file / `--stat` / `--numstat` / helper diff-header summaries 必须先 count 再 cap。
- 不要手写或复用删减版 `Evidence-budget contract`；如果需要改写 prompt，必须复制 reference 里的完整硬约束清单，而不是只写 `avoid huge diffs`、`focused hunks` 或部分禁令。英文 focus-heavy prompt 也必须保留完整 contract；不能只写 `You are an independent code reviewer` 加 focus areas。review-only 子线程里的 validation command 也要有输出预算：不要用 `max_output_tokens=60000` / `max_output_tokens=100000` 直接启动完整 test/build；先用小 probe 或低可见输出预算，遇到 sandbox tempdir、pyenv shim 或 repeated `unittest` `E` 输出时先摘要失败形态，再升级或落 task-scoped log。
...
- 非 Codex reviewers（OpenCode、Cursor `agent`、Copilot、Claude 等）只在 Joey 明确要求且 egress/consent rules allow it，或当前任务显式 opt-in 时运行；本地双重 review / triple review 请求固定包含该 non-Codex external half，但缺少 non-Codex consent 时只能报告该 half blocked，不要运行它，也不要静默降级为 Codex-only triple review。
...
- 只有 required review gates、所有 requested review lanes（包括 requested 但因 consent/auth/runtime 缺失而 blocked 的 lanes）、required CI、required conversation resolution 和 branch/base 状态 clean，或 Joey 明确接受例外后，才报告 merge-ready。
