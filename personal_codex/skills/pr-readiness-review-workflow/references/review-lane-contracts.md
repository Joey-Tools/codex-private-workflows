...
For helper-managed Codex lanes, record the final model in the parent gate evidence. The default model policy is `gpt-5.6-sol` with `xhigh`, with one automatic retry on `gpt-5.5` with `xhigh` only when the primary model is unavailable, unsupported, not found, overloaded/capacity-limited, or not entitled. Generic auth, network, sandbox, timeout, validation, and reviewer-result failures are not model-fallback conditions.
