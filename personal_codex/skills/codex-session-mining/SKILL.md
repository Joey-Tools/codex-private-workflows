...
- Session-derived retained or report JSONL artifacts when Joey explicitly scopes the task to those artifacts instead of raw rollouts.
...
- Unknown mappings, incomplete or unsafe rollout evidence, no usable complete pair, or a tie without reliable turn ordering produce the full `GPT-5.6 Terra Ultra` fallback sentence.
