...
PERSONAL_AGENTS_CURRENT_REVIEW_BLOCK = (
    ...
    b"mutation. Bare triple authorizes at most one possibly delivered scoped exact "
    b"`@codex review` issue-comment POST for one repository/PR/head epoch on an "
    b"already-existing eligible PR. Once that call could have reached GitHub, it "
    b"consumes the epoch's comment-mutation budget regardless of the client result; "
    b"an ambiguous outcome permits only a complete reread of the unchanged head and "
    b"visible request set followed by observation and, if delivery remains unproved, "
    b"an inconclusive result\xe2\x80\x94never another comment POST in that epoch. Any GitHub "
    b"Actions rerun, dispatch, or reconciliation requires separate current-task "
    b"delivery or readiness authorization plus one frozen exact tuple of repository, "
    b"PR, head, Action or workflow, operation, and inputs. Repetition is allowed only "
    b"while that tuple remains type-preservingly unchanged; the same tuple is "
    b"idempotent for this consumer without a repository predeclaration, while a "
    b"changed tuple or different workflow, scope, destination, PR, repository, or "
    b"unrelated action requires ordinary confirmation.\n"
)
