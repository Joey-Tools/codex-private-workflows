...
- PR-bound delivery in the listed Joey-Tools skill repositories uses exactly one fresh-context local Codex reviewer and one current-head GitHub `@codex review` lane. Claude Code remains outside this custom repository-family gate unless Joey explicitly opts in for the current task.
...
- Update the skill-repository list only when a new canonical Joey-Tools owner is added or an existing repository changes ownership.
