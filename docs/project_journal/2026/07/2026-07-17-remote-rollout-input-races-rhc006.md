...
- Made `session-meta` accept only complete newline-terminated JSONL records inside its byte cap and propagate directory-enumeration failures through path-neutral errors while preserving optional directories that disappear during enumeration.
...
- Keep ancestor-swap, complete-record boundary, directory-enumeration, and parent-capture budget regressions in the directly affected probe suite.

## Evidence

- Complete remote probe regression module: 51 tests passed.
...
- Complete repository regression suite: 606 tests passed, 1 skipped.
