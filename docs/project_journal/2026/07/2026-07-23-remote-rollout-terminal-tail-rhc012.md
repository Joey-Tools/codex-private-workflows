...
- Output publication retains and revalidates the created file object across descriptor-relative replacement, including exact content readback, and uses a random 128-bit temporary name. The receipt ends at the final target-entry check; a separate same-UID writer can still mutate the destination afterward.
