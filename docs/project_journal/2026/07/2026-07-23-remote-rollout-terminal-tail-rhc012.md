...
- Output publication uses a random 128-bit temporary name, validates exact bytes before descriptor-relative replacement, and retains/revalidates the created file object across that replacement. The receipt requires cooperative same-UID processes; hostile same-UID races against the temporary or newly visible destination are explicitly outside the guarantee.
