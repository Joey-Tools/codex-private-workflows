...
- The regular-file overlay rejects unsafe paths, symlinks, hard links, missing or non-regular inputs and targets, oversized sources, unsafe ownership or permissions, duplicate outputs, and observed path or file-identity drift. Source and target traversal is anchored to canonical root directory descriptors and opens every descendant component with `O_DIRECTORY|O_NOFOLLOW`; unavailable secure primitives fail closed.
...
- A current-base independent pass then identified that resolving the complete overlay root inside the secure opener could follow a root symlink introduced after preflight. Production roots are canonicalized once at sync entry, while the opener now preserves that lexical root and traverses it from `/` with `O_DIRECTORY|O_NOFOLLOW`; deterministic source-root and target-root swap tests fail closed without touching the outside sentinel. The complete private suite passed with the sandbox-only Git signing override used by its ephemeral merge fixtures (`494` tests, `2` skipped).
