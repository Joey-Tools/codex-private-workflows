...
- A current-base independent pass then identified that resolving the complete overlay root inside the secure opener could follow a root symlink introduced after preflight. A follow-up pass proved that no-follow traversal alone could still reopen a replacement ordinary directory. The final opener pins the raw root descriptor before preflight, derives all I/O from it, and compares pre/post secure-reopen root identities; deterministic symlink, ordinary-directory, preflight, and post-binding source/target swaps all fail closed while recorded I/O remains bound to the original inode. The complete private suite passed with the sandbox-only Git signing override used by its ephemeral merge fixtures (`498` tests, `2` skipped).
