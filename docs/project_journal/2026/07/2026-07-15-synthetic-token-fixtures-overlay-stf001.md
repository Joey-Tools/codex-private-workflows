...
- The final helper-backed pass identified one remaining stale-name cleanup race before installation: a failed temporary rename could previously unlink an unrelated file rebound at the temporary basename. Preparation failures now retain the original temporary inode and any rebound basename inside the bounded recovery scope instead of deleting by pathname. A deterministic full-sync regression moves the original inode, installs an unknown replacement at the old name, forces rename failure, and proves both files and the prior installed target remain intact. The focused sync suite passed again (`87` tests).
