...
- On macOS the source-lock verifier ignores a PATH-selected Homebrew convenience symlink and executes fixed `/usr/bin/git`. It binds that root-owned regular executable and every root-owned, non-group/world-writable ancestor by object identity and access policy before and after each command. Current-user-owned `0775` Homebrew ancestors are intentionally outside this trust path rather than newly admitted.
...
- The sole consumable named single on `e1b85cfec954053d8d00a9bb61197a09ed7959d0` found that `shutil.which("git")` selected `/opt/homebrew/bin/git`, whose symlink entry was rejected before source verification. The superseding repair narrows macOS execution to fixed `/usr/bin/git`; its protected property is the exact executable object's identity plus owner/group/mode access policy, not PATH-entry stability. Because the full root-owned ancestor chain is non-writable by untrusted principals, the final check-to-exec interval does not admit an untrusted replacement; root compromise remains outside the stated property.
- The repaired source-lock module passed 40/40 tests with one expected skip under both CI-compatible Python 3.10 and Python 3.13 while PATH preferred `/opt/homebrew/bin`; focused coverage proves macOS executes `/usr/bin/git`, non-macOS does not broaden trust to the `0775` Homebrew chain, a world-writable ancestor fails closed, and final-binary replacement invalidates the binding.
