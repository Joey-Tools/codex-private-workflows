...
- Bound each reader to the descriptor-derived source size and the handle's validated starting byte offset. A scan cap that cuts through a record drops the incomplete buffer, while a stable true EOF still preserves a complete final JSON record without LF.
...
- Explicit and inferred source-size paths both measure scan limits and true EOF from the current byte cursor; missing, negative, or out-of-range offsets fail closed.
...
- `python3 -m unittest tests/test_remote_codex_probe.py -q`: 100 tests passed in 3.267 seconds.
- The local/embedded regressions cover oversized 64 KiB cross-chunk bare-CR drain, LF/CRLF/cap/EOF boundary behavior, safe `BytesIO` snapshot-size inference, nonzero-cursor true EOF and unread-suffix caps, and unavailable or invalid offset rejection.
...
- Full private suite with process-local Git signing disabled: 658 tests passed in 73.208 seconds.
